...
export async function GET(req: NextRequest) {
  ...
  if (!token) return NextResponse.json({ profile: null }, { status: 400 });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!token || !display_name?.trim()) return NextResponse.json({ error: "缺少必要欄位" }, { status: 400 });
    ...
    if (!profile) {
      ...
      if (Object.keys(updates).length > 0) await updateUserProfile(token, updates);
      profile = await fetchUserProfile(token);
    }
    return NextResponse.json({ profile }, { status: profile ? 200 : 500 });
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!adminProfile || !canModerate(adminProfile.role)) return NextResponse.json({ error: "權限不足" }, { status: 403 });
    if (action === "set_role") {
      if (adminProfile.role !== "admin") return NextResponse.json({ error: "只有管理員可設定角色" }, { status: 403 });
