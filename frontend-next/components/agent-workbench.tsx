          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-4 text-sm text-zinc-300">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <span>Role</span>
                <span className="text-right text-cyan-200">{role.shortLabel}</span>
                <span>Provider</span>
            ...
            {IDOL_AGENT_ROLES.map((item) => (
              <button
                ...
                onClick={() => {
                  ...
                  setQuestion(item.defaultQuestions[0]);
                }}
              ...
              >
                <div className="text-xs uppercase tracking-[0.18em] text-cyan-300">{item.shortLabel}</div>
                ...
                <div className="mt-3 text-xs text-zinc-500">Source: {item.sourceAgentName}</div>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-5">
          <div className="flex flex-col gap-3">
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-zinc-300">
              <div className="text-xs uppercase tracking-[0.18em] text-cyan-300">Current Role</div>
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-xs text-cyan-100">
                {role.focus.map((item) => (
                  <span key={item} className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1">
                    {item}
                  </span>
                ))}
              </div>
            ...
            <textarea
              ...
              placeholder="輸入你要問的平台問題，例如某位成員為何上榜、公式如何解釋、需要補哪些社群欄位，或下一步產品方向。"
            />
            <div className="flex flex-wrap gap-3">
              ...
              {role.defaultQuestions.map((preset) => (
                <button
                  key={preset}
                  type="button"
                  onClick={() => ask(preset)}
          ...
          <section className="space-y-6">
            {result ? (
              ...
              <div className="rounded-3xl border border-dashed border-white/10 bg-white/5 p-6 text-sm leading-7 text-zinc-300">
                先選一個角色再提問。相同問題可以換不同角色重問，例如用 Ranking Analyst 解榜單，再用 Formula Architect 看權重，
                或用 Data Pipeline Operator 找缺資料原因。
              </div>
            )}
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
                ...
                <p className="mt-2 text-sm text-zinc-400">直接比較兩個 entity 的分數與更新天數差距。</p>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
              ...
              {result?.evidence?.length ? (
                ...
                <p className="mt-3 text-sm text-zinc-400">送出問題後，這裡會顯示本次回答引用到的角色、排行資料、insights 與 brain 線索。</p>
              )}
            </div>
          </section>
          ...
          <aside className="space-y-6">
                  ...
                  watchedEntities.map((entity) => (
                    <div key={entity.id} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        <button type="button" onClick={() => void addAlert(entity, "分數下降 5 分以上")} className="rounded-lg border border-pink-400/20 bg-pink-400/10 px-3 py-2 text-xs text-pink-100">
                          分數下滑
                        </button>
                        <button type="button" onClick={() => void addAlert(entity, "14 天沒有新的資料更新")} className="rounded-lg border border-amber-400/20 bg-amber-400/10 px-3 py-2 text-xs text-amber-100">
                          更新停滯
                        </button>
                      </div>
                    </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
              ...
              <div className="mt-4 space-y-3">
                ...
                {!result?.traces?.length ? <p className="text-sm text-zinc-400">送出問題後，這裡會顯示本次實際用到的角色與資料工具。</p> : null}
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
              ...
              <div className="mt-4 flex flex-col gap-3">
                {(result?.suggestedQuestions || role.defaultQuestions).map((item) => (
                  <button
                    key={item}
                    type="button"
                    onClick={() => ask(item)}
